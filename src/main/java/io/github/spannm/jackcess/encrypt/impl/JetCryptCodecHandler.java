...
public class JetCryptCodecHandler extends BaseJetCryptCodecHandler {
    static final int ENCODING_KEY_LENGTH = 0x4;
    ...
    public static CodecHandler create(PageChannel channel) throws IOException {
        ...
        byte[] encodingKey = ByteUtil.getBytes(buffer, format.OFFSET_ENCODING_KEY, ENCODING_KEY_LENGTH);

        return isBlankKey(encodingKey) ? DefaultCodecProvider.DUMMY_HANDLER : new JetCryptCodecHandler(channel, encodingKey);
    }
